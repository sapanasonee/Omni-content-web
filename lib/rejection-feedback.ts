...
export interface RejectionReasonDef {
  ...
  label: string
  // The directive injected into generation to fix this reason. Written as an
...
export const REJECTION_REASONS: readonly RejectionReasonDef[] = [
  {
    ...
    correction:
      "Match the founder's voice and tones exactly — recent drafts drifted off their register. Reread the VOICE section and stay inside it.",
  },
  {
    value: 'too_flat',
    label: "It's too flat",
...
// Aggregate reasons across recent rejections and return the corrective
// directives for any reason at/above the recurrence threshold. Pure — the
// caller supplies the records (most-recent-first order doesn't matter here).
// Returned directives follow REJECTION_REASONS order for stable output.
export function recurringCorrectives(rejections: StoredRejection[]): string[] {
  ...
  return REJECTION_REASONS
    .filter(r => (counts.get(r.value) || 0) >= RECURRENCE_THRESHOLD)
    .map(r => r.correction)
}
